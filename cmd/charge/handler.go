...
func handlerRequest(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	tokenRepository := adapters.NewTokenDynamoRepository(client)
	tokenUseCase := usecases.NewTokenUseCase(tokenRepository)
	bankMock := adapters.NewMockBankGateway()
	chargeUseCase := usecases.NewChargeUseCase(chargeRepository, tokenUseCase, bankMock)

	var chargeRequest dtos.ChargeDTO
	...
	charge, err := chargeUseCase.CreateCharge(chargeRequest)
